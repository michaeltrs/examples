...
parser.add_argument('--num_classes', default=1000, type=int,
                    metavar='N',
                    help='number of output classes in dataset')
parser.add_argument('--lambda1', default=0.1, type=float, metavar='lam',
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    classifier = FlexiSoftmaxClassifier(args.num_classes)
    ...
    if args.evaluate:
        validate(val_loader, model, criterion, args, abs_step=None)
        return

    for epoch in range(args.start_epoch, args.epochs):
        if args.distributed:
            train_sampler.set_epoch(epoch)
        adjust_model_learning_rate(optimizer, epoch, args)
        adjust_classifier_learning_rate(optimizerR, epoch, args)
...
def train(train_loader, model, classifier, criterion, optimizers, epoch, args):
    ...
    losses_ce = AverageMeter('LossCE', ':.4e')
    losses_ce_flexi = AverageMeter('LossFCE', ':.4e')
    losses_penalty = AverageMeter('LossPen', ':.4e')
...
def validate(val_loader, model, criterion, args, abs_step):
    batch_time = AverageMeter('Time', ':6.3f')
    # losses = AverageMeter('Loss', ':.4e')
    losses_ce = AverageMeter('LossCE', ':.4e')
    # losses_ce_flexi = AverageMeter('LossFCE', ':.4e')
    ...
    progress = ProgressMeter(
        len(val_loader),
        [batch_time, losses_ce, top1, top5],
        # [batch_time, losses, losses_ce, losses_ce_flexi, losses_penalty, top1, top5],
        prefix='Test: ')
...
def adjust_model_learning_rate(optimizer, epoch, args):
...
def adjust_classifier_learning_rate(optimizer, epoch, args):
    """Sets the learning rate to the initial LR decayed by 10 every 30 epochs"""
    lr = args.lrR * (0.1 ** (epoch // 30))
    for param_group in optimizer.param_groups:
        param_group['lr'] = lr
